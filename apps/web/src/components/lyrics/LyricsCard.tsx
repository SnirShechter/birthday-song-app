...
function formatLyrics(text: string): ReactNode[] {
  return text.split("\n").map((line, i) => {
    ...
    if (/^\[.*\]$/.test(trimmed)) {
      return (
        <p
          key={i}
          className="mt-3 mb-1 text-xs font-bold uppercase tracking-wider text-[var(--color-primary)]"
        >
          {trimmed}
        </p>
      );
  ...
  return (
    ...
    >
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="rounded-full bg-[var(--surface)] px-2.5 py-0.5 text-xs font-medium text-[var(--text-muted)] border border-[var(--border)]">
        ...
        {isSelected && (
          <motion.span
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="ml-auto flex h-6 w-6 items-center justify-center rounded-full bg-[image:var(--gradient-main)]"
          >
        ...
        <AnimatePresence mode="wait">
          {isEditing ? (
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="max-h-[320px] overflow-y-auto rounded-lg text-sm text-[var(--text)] pr-1"
            >
              {formatLyrics(displayContent)}
            </motion.div>
          )}
        </AnimatePresence>
      </div>
        ...
        <div className="flex items-center gap-2">
          <motion.button
            ...
            className={cn(
              ...
              "hover:text-[var(--text)] hover:border-[var(--color-primary)]/30",
          ...
          <motion.button
            ...
            disabled={isSelected}
            className={cn(
              "flex flex-1 items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold",
              "transition-all duration-200 cursor-pointer",
              isSelected
                ? "bg-[var(--color-success)]/15 text-[var(--color-success)] border border-[var(--color-success)]/30 cursor-default"
                : "bg-[image:var(--gradient-main)] text-white shadow-lg shadow-[var(--color-primary)]/25 hover:shadow-xl hover:brightness-110"
            )}
          >
            {isSelected ? (
              <>
